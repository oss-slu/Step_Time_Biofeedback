...
function App() {
  ...
  const [currentPatientView, setCurrentPatientView] =
    useState("StanceTimeDigits");
  ...
  const views = {
    ...
    StanceTimeDigits: (
      <StanceTimeDigits
        stanceTime={stanceTime}
        data-testid="stance-time-digits-view"
      />
    ),
    StanceTimeChart: (
      <StanceTimeChart
        stanceTime={stanceTime}
        data-testid="stance-time-chart-view"
      />
    ),
    StanceTimeGraph: (
      <StanceTimeGraph
        stanceTime={stanceTime}
        data-testid="stance-time-graph-view"
      />
    ),
  };
  ...
  function updateVisualThreshold(forceData) {
    let color = null;
    if (forceData <= 19.0 && forceData >= 18.0) {
      color = "yellow";
    } else if (forceData < 18.0) {
      color = "red";
    } else {
      color = "green";
    }
  ...
  const reconnectWebsocket = useCallback(() => {
    ...
    websocket.current.onerror = (event) => {
      ...
      setWebSocketError(
        "WebSocket encountered an error. Data streaming stopped."
      );
    };
  }, []);
        ...
        <div className="main-view">
          <img
            ...
            style={{
              transform: `rotate(${
                patientWindow &&
                !patientWindow.closed &&
                currentPatientView === currentView
                  ? "180deg"
                  : "0deg"
              })`,
            }}
          ></img>
          <header className="App-header">{views[currentView]}</header>
        </div>
      </div>
    </div>
  );
